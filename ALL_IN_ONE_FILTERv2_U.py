...
cameraMatrix_O = np.zeros((3, 3), dtype="float64")
distCoeffs = np.zeros((8, 1), dtype="float64")
roi = [1, 1, 1, 1]
has_cam_info = False
def camera_info_clb(msg):
    global has_cam_info, cameraMatrix, distCoeffs, cameraMatrix_O, roi
    if not has_cam_info:
        # конвертация параметров камеры из формата ROS в OpenCV
        has_cam_info = True
        distCoeffs = np.array(msg.D, dtype="float64")
        cameraMatrix_O = np.reshape(np.array(msg.K, dtype="float64"), (3, 3))
        w = 320
        h = 240
        cameraMatrix, roi = cv2.getOptimalNewCameraMatrix(cameraMatrix_O, distCoeffs, (w, h), 1, (w, h))
...
def img_clb(msg):
    global has_cam_info, cameraMatrix, distCoeffs, result_GLOBAL, circles_GLOBAL, cameraMatrix_O, roi
    image = bridge.imgmsg_to_cv2(msg, "bgr8")
    image = cv2.undistort(image, cameraMatrix_O, distCoeffs, None, cameraMatrix)
    # x, y, w, h = roi
    ...
    if has_cam_info:
        for r in result_in_img_frame:
            result_GLOBAL.append(get_rect_pose(r, objectPoint, cameraMatrix, np.zeros((8, 1), dtype="float64")))
        for c in circles_in_img_frame:
            circles_GLOBAL.append(get_rect_pose(c, objectPoint_circles, cameraMatrix, np.zeros((8, 1), dtype="float64")))
        # if len(result) > 0:
        #     print("RES: \n " + "\n ".join(map(str, result)))
        # if len(circles) > 0:
